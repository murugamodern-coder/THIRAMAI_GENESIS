...
export default function BrainResponseBlock({ brain, className = "" }) {
  ...
  return (
    <div className={`space-y-3 rounded-xl border border-slate-700/80 bg-slate-950/80 p-4 text-[#e2e8f0] ${className}`.trim()}>
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-800 pb-3">
        <span className="text-xs font-semibold uppercase tracking-wide text-slate-200">Brain</span>
        ...
        {result?.run_id != null ? (
          <span className="text-[11px] text-slate-200">run #{String(result.run_id)}</span>
        ) : null}
      </div>

      <section>
        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-white">Plan</h4>
        {plan.length === 0 ? (
          <p className="text-sm text-slate-200">No plan steps.</p>
        ) : (
              ...
              return (
                <li key={`p_${o}_${i}`} className="rounded-lg border border-slate-800 bg-slate-900/40 px-3 py-2">
                  <div className="font-medium text-slate-100">
                    <span className="text-slate-200">#{o}</span> {ph ? `${ph} · ` : ""}
                    {sk}
                  </div>
                  {tier || score ? (
      ...
      <section>
        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-white">Execution</h4>
        {stopped ? (
        ...
        {stepsOut.length > 0 ? (
              ...
              return (
                <li
                  ...
                  className={`flex items-start gap-2 rounded-lg border px-3 py-2 text-sm ${
                    ok ? "border-slate-800 bg-slate-900/80 text-[#e2e8f0]" : "border-red-900/80 bg-red-950/80 text-red-100"
                  }`}
                >
                  <span className="mt-0.5 text-slate-200">#{String(s?.step_order ?? i)}</span>
                  <div>
                    <div className="font-medium">{String(s?.step_kind || "step")}</div>
                    {s?.skipped ? <div className="text-xs text-slate-200">skipped</div> : null}
                  </div>
                </li>
              );
            })}
          ...
          <pre className="max-h-40 overflow-auto rounded-lg bg-slate-900 p-3 text-xs text-[#e2e8f0]">{safeJson(result)}</pre>
        ) : null}
      </section>

      <details className="rounded-lg border border-slate-800 bg-slate-900/30">
        <summary className="cursor-pointer px-3 py-2 text-xs text-slate-200">Raw response</summary>
        <pre className="max-h-48 overflow-auto border-t border-slate-800 p-3 text-xs text-white">{safeJson(brain)}</pre>
      </details>
    </div>
  );
}
